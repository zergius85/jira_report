    ...
    if not df_detail.empty:
        ...
        if not df_detail.empty:
            df_assignees = df_detail.groupby('РСЃРїРѕР»РЅРёС‚РµР»СЊ').agg(
                tasks_count=('РљР»СЋС‡', 'count'),
                correct_count=('РџСЂРѕР±Р»РµРјС‹', lambda x: (x == '').sum()),
                issues_count=('РџСЂРѕР±Р»РµРјС‹', lambda x: (x != '').sum()),
                fact_sum=('Р¤Р°РєС‚ (С‡)', 'sum'),
                estimate_sum=('РћС†РµРЅРєР° (С‡)', 'sum')
            ).reset_index()
            # РџРµСЂРµРёРјРµРЅРѕРІС‹РІР°РµРј РєРѕР»РѕРЅРєРё РѕР±СЂР°С‚РЅРѕ РІ РєРёСЂРёР»Р»РёС†Сѓ РґР»СЏ РѕС‚РѕР±СЂР°Р¶РµРЅРёСЏ
            df_assignees = df_assignees.rename(columns={
                'tasks_count': 'Р—Р°РґР°С‡',
                'correct_count': 'РљРѕСЂСЂРµРєС‚РЅС‹С…',
                'issues_count': 'РЎ РѕС€РёР±РєР°РјРё',
                'fact_sum': 'Р¤Р°РєС‚ (С‡)',
                'estimate_sum': 'РћС†РµРЅРєР° (С‡)'
            })
